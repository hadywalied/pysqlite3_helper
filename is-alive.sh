...
function IsProcessRunning() {
  if ps -p $1 >/dev/null; then # process is running
    return 0
...
function ParseProcessName() {
  ...
  for fcount in $(pgrep -u $USER -x $1); do
    pidList+=("$fcount")
    printf "$fcount "
  done
  echo "]"
...
PrintPid() {
  ...
  for ((c = 1; c <= $lvl; c++)); do #c-> numberoftabs
    printf "\t"
  done
  ...
  local memShrd=$(($(pmap -d $pid | tail -1 | awk '{print $NF}' | grep -o '[[:digit:]]*') / 1024)) #in MB
